...
It is mostly used to replace `map()` and `filter()` functions when the function we want to apply is a simple lambda function.
...
> **NOTE 1**: You can stack many `for` and `if` statements in a list comprehension.
> They will be executed in the same order as they would be in a normal `for` loop.
